...
void scanline_convert( struct matrix *points, int i, screen s, zbuffer zb ) {
  ...
  //declare vars
  color c;
  int top, mid, bot, temp, y, topy, midy, boty;
  double topx, midx, botx, topz, midz, botz,
    x0, z0, x1, z1, dx0, dz0, dx1, dz1;
  ...
  //init point pos in *points
  top = i;
  mid = i + 1;
  bot = i + 2;

  //sort points by ycor to get actual top/mid/bot
  ...
  // [ |0| /1/ \2\ ] comp 1 and 2
  if ( points->m[1][mid] < points->m[1][bot] ) {
    temp = bot;
    bot = mid;
    mid = temp;
  }

  // [ /0/ \1\ |2| ] comp 0 and 1 again
  if ( points->m[1][top] < points->m[1][mid] ) {
    temp = mid;
    mid = top;
    top = temp;
  ...
  //printf("%f\t%f\t%f\n", points->m[1][top], points->m[1][mid], points->m[1][bot]);

  //assign vals
  topx = points->m[0][top]; topy = (int) points->m[1][top]; topz = points->m[2][top];
  midx = points->m[0][mid]; midy = (int) points->m[1][mid]; midz = points->m[2][mid];
  botx = points->m[0][bot]; boty = (int) points->m[1][bot]; botz = points->m[2][bot];

  //printf("%f\t%f\t%f\n", topy, midy, boty);

  //init draw_lines vals
  x0 = x1 = botx;
  y = (int) boty;
  z0 = z1 = botz;
  ...
  //printf("%f triggered d0 inits 2\n", topy - boty);
  dx0 = (topx - botx) / (topy - boty);
  dz0 = (topz - botz) / (topy - boty);
  
  //init dx1, dz1
  dx1 = dz1 = 0;

  if ( midy - boty != 0.0 ) {
    //printf("%f triggered d1 inits 1\n", midy - boty);
    dx1 = (midx - botx) / (midy - boty);
    dz1 = (midz - botz) / (midy - boty);
  }

  //printf("%f\t%f\t%f\t%f\n", dx0, dz0, dx1, dz1);

  //draw lines up to midpoint
  for (y = boty; y <= midy; y++) {
  ...
  //switch dx1, dz1 at midpoint for new set of lines
  dx1 = dz1 = 0;
  x1 = midx;
  z1 = midz;
  
  if ( topy - midy != 0 ) {
    //printf("%f triggered d1 inits 2\n", topy - midy);
    dx1 = (topx - midx) / (topy - midy);
    dz1 = (topz - midz) / (topy - midy);
  }

  //draw lines up to top point
  for (y = (int) midy; y < topy; y++) {
    draw_line( x0, y, z0, x1, y, z1, s, zb, c);
    x0 += dx0;
    z0 += dz0;
    x1 += dx1;
...
void draw_line(int x0, int y0, double z0,
  ...
  /* THIS IS WHAT IM ADDIN YAH */
  if ( loop_start != loop_end ) {
    dz = ( z1 - z0 ) / ( abs(loop_end - loop_start) );
  }
